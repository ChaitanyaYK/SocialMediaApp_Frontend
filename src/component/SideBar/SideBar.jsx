...
import {Home, LogOut, Menu, Settings, User, LogIn, TvMinimalPlayIcon} from "lucide-react";
...
const SideBar = () => {
   ...
   const navItems = [
    ...
    { name: "Settings", icon: Settings, path: "/updateAccount" },
  ...
  return (
    ...
    <div className={`grid grid-rows-3  ${open ? "w-66": "w-19"} transition-transform ease-in-out to-0% to-50% 1s relative bg-gray-200`}>
        {/* <div className={` bg-black shadow-md transition-all duration-300 flex flex-col`}> */}
          <Button onClick={() => setOpen(!open)} className="w-full h-10">
           < Menu />
          </ Button>
        ...
        {navItems.map(({ name, icon: Icon, path }) => (
          <Link
            to={path}
            key={name}
            className="flex items-center gap-3 px-4 py-2 rounded-sm text-gray-300 hover:bg-gray-800 border-4 border-b-gray-600 border-l-gray-800 hover:text-white transition"
          >
